...
LLMモデル変更を含む再評価では比較CLIに `--allow-model-difference` を追加します。入力・ケース順・Jev/Embedding/Moderation設定・採点指示の一致と各runの実モデル記録は検査し続けます。比較結果の差はモデル変更とアプリ改修が重なるため、改修単独の効果と断定しません。
