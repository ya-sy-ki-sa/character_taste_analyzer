...
import { qualityReportSchema } from "./report";
...
export async function runQualityEvaluation(
  ...
  for (const fixture of qualityCases
    ...
    try {
      ...
      let generationSkippedReason: "no_preferences" | "no_selected_items" | undefined = options.generate
        ? "no_preferences"
        : undefined;
      if (options.generate && detail.preferenceAnalysis.assertions.length) {
        ...
        generationSkippedReason = "no_selected_items";
        if (selectedItemIds.length) {
          generationSkippedReason = undefined;
  ...
  return qualityReportSchema.parse({
    schemaVersion: "2.0",
    generationRequested: Boolean(options.generate),
