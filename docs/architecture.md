...
意味判定は `worker/judgment` の独立したJudgmentProviderを使用します。通常版・dark版とも、LLMの候補発見・生成にJevの対象・根拠・条件・採点を組み合わせ、コードが再検討と保留を制御します。生成案の検証・順位付けもJevへ統一します。旧方式との切替フラグはありません。

中間判定はメモリ内だけで保持し、既存のZod契約へ変換して保存します。D1/R2、保存JSON、公開API、エクスポート形式を変更しません。JevをLLMの実行記録へ偽装せず、Jevの利用量・判定区分・失敗は運用ログに記録します。実際に呼び出したLLMの原出力ハッシュは引き続き保持します。

詳細と検証範囲は [Jev処理](jev-pipeline.md) を参照してください。
...
通常版の反応経路の定義は `shared/response-channels.ts` を正本とし、抽出・監査の共通system指示へ1回ずつ組み込みます。好意と規範的賛否、憧れと同一化願望、主観的な自己照合と人物の事実の境界も共有します。
...
通常版の補完判定は、意味監査結果の根拠検証・正規化後に行います。判定用と保存用の出典準備は `analysis/understanding-provenance.ts`、根拠検証と正規化は同じ関数を使います。判定用処理はDBへ書き込まず、最終的な出典・人物像・実行記録は世代を確認する同一batchで保存します。補完判定や保存用の正規化で、実行記録の原出力とハッシュを書き換えません。検証中の失敗にも、それまでの呼び出し記録を引き継ぎます。
