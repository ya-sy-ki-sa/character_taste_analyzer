...
エクスポートは `POST /api/v1/account/exports` で作成し、同じリソースのID配下で状態を確認します。readyになったファイルだけを所有者認証付きでダウンロードします。スキーマは5.0です。作成済みの4.0ファイルは当時の形式のまま取得できます。ジョブ失敗は状態・安全なエラーコードを確認して再試行します。
...
## 人物理解の全体信頼度廃止（migration 005）

`005_retire_overall_confidence.sql` は既存の `overall_confidence` をNULL許容の `legacy_overall_confidence` へコピーして旧列を削除します。旧指標は履歴用であり、現在の情報量や正確性を示しません。新規スナップショットではNULLです。過去データの再分析・補正はしません。

この移行は旧アプリの書き込みと互換性がありません。通常の稼働中ローリング更新として適用しないでください。

1. 対象環境のバックアップを確保し、新規解析・再分析・確認編集・エクスポートを受け付ける書き込み経路を停止する。
2. 実行中のジョブ・Workflow・outbox処理の完了を確認して処理を停止する。旧コードの書き込みが再開しない状態にする。
3. migration 005を適用し、対応アプリを配置する。
4. 新規解析、レビュー取得、基本像・対象像、エクスポートの版と内容を確認して処理・受付を再開する。

旧アプリだけを戻すと削除済みの列へ書き込んで失敗します。復旧は対応するDBバックアップとアプリを組にして戻すか、対応アプリの修正版を適用します。remote DBへの適用とデプロイは別途実施する運用作業です。
