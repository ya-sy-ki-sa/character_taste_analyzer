...
新規に抽出・監査する嗜好候補は、[属性命名プロンプト](../worker/llm/prompts/preference-attributes.ts)の共通ルールと該当版のルールに従います。嗜好プロンプトは `v3.8.0`、仮説提案は `v2.5.0` です。追加質問後の抽出・監査と、新規の仮説提案にも同じ命名方針を適用します。
