...
briefCoverageは各selectionを一度ずつ含め、反映先JSON Pointerを正確に返す。Pointerのルートは生成人物自身であり、/personality/summaryのように書く。/candidateや/characterという包みの階層を付けない。responseChannel=nullは反応経路未確定を意味する。具体的な好みと条件は反映し、反応経路を推測で補わない。指定JSON Schemaだけを返す。`;
...
briefCoverageは各selectionを一度ずつ含める。Pointerのルートは生成人物自身であり、/darkCore/narrativeFunctionなどを使う。/candidateや/characterという包みの階層を付けない。responseChannel=nullは反応経路未確定を意味する。具体的な好みと条件は反映し、反応経路を推測で補わない。指定JSON Schemaだけを返す。`;
