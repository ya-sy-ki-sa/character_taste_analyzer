...
briefCoverageは各selectionを一度ずつ含め、反映先JSON Pointerを正確に返す。Pointerのルートは生成人物自身であり、/personality/summaryのように書く。/candidateや/characterという包みの階層を付けない。responseChannel=nullは反応経路未確定を意味する。具体的な好みと条件は反映し、反応経路を推測で補わない。否定を含む対象の記述と評価の正負を別々に読み、条件・例外・人物間の行為の向き・ユーザーの過去と現在を保持する。条件付きの苦手を無条件の禁止へ広げず、公式設定とユーザーの関係解釈を混同しない。指定JSON Schemaだけを返す。`;
...
briefCoverageは各selectionを一度ずつ含める。Pointerのルートは生成人物自身であり、/darkCore/narrativeFunctionなどを使う。/candidateや/characterという包みの階層を付けない。responseChannel=nullは反応経路未確定を意味する。具体的な好みと条件は反映し、反応経路を推測で補わない。否定を含む対象の記述と評価の正負を別々に読み、条件・例外・人物間の行為の向き・ユーザーの過去と現在を保持する。条件付きの苦手を無条件の禁止へ広げず、公式設定とユーザーの関係解釈を混同しない。指定JSON Schemaだけを返す。`;
