export const SEMANTIC_AUDIT_POLICY = "semantic-integrity/v1.1.0";
export const SEMANTIC_AUDIT_SCHEMA_VERSION = "1.0";
const SEMANTIC_AUDIT_COMMON = `改訂した最終候補それぞれにscopeAssessment、各evidenceにsupportAssessmentを付けてください。判定は改訂前ではなく、実際に返すラベル・描写・極性・条件・例外・要約の意味に対して行います。
...
partialは同じ監査内で主張を支持範囲へ狭め、狭めた最終主張を再判定してください。根拠のない部分を消せなければpartialのままです。原文・出典本文がなく照合できない情報はunverifiableです。
...
確認済み人物理解は人物理解の資料であり、ユーザーがその属性を好きだという根拠ではありません。モデル知識からユーザーの好みを作らず、無効な出典をモデル知識へ読み替えてはいけません。
