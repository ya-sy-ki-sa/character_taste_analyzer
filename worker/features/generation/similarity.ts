...
import { embedSimilarityDocuments } from "./embedding";
import * as repository from "./repositories/similarity";
...
export async function loadSimilarityDocuments(
  ...
  const [registered, generated] = await Promise.all([
    all<{ id: string; summary_json: string; character_name: string }>(
      repository.selectCharacterAssertions(env.DB, [ownerUserId, domain]),
    ),
    all<{ id: string; character_json: string }>(
      repository.selectGeneratedCharacters(env.DB, [ownerUserId, domain, requestId, ownerUserId, domain, requestId]),
    ),
  ]);
  return [
    ...registered.map((row) => {
  ...
  if (semantic) {
    ...
    for (const document of [...documents, current]) {
      ...
      const cached = await first<{ vector_json: string }>(
        repository.selectCharacterSimilarityDocuments(env.DB, [
          ownerUserId,
          document.id,
          hash,
    ...
    for (let offset = 0; offset < missing.length; offset += 20) {
      ...
      await env.DB.batch(
        batch.map((document) =>
          repository.insertCharacterSimilarityDocuments(env.DB, [
            crypto.randomUUID(),
            ownerUserId,
            document.id,
            document.hash,
