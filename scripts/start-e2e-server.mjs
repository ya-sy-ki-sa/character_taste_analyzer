...
if (existsSync(offlineVarsPath)) {
  if (!readFileSync(offlineVarsPath, "utf8").startsWith(offlineVarsHeader))
    throw new Error(".dev.vars.offline exists and is not managed by the E2E launcher");
  unlinkSync(offlineVarsPath);
}
writeFileSync(offlineVarsPath, `${offlineVarsHeader}AUTH_PEPPER=offline-e2e-only-deterministic-pepper-2026\n`, {
  mode: 0o600,
});
function removeOfflineVars() {
...
process.on("exit", removeOfflineVars);
